...
[1242](https://www.acmicpc.net/problem/1242) : 2021.04.07.

- 리스트를 다루는 문제인 것 처럼 보이는 수학문제
- 그대로 시키는대로 하면 안되는 문제였다
- 주어진것들이 도저히 안될것같으면, 잘 변형해볼 것
  - 이 문제에서는, 학생의 번호(바뀌지않는것) -> 학생의 자리(옮기면서 바로 찾을 수 있는 것)

[20057](https://www.acmicpc.net/problem/20057) : 2021.07.22.

- 오랜만에 문제풀이
- 달팽이수열 비슷한거랑 이상한 숫자계산이 섞여있는 문제
- 그냥 시키는대로만 했더니 채점이 너무 느리게 올라가서 시간초과 날줄 알았는데 통과했다
- 똑같은 코드를 줄이는 연습을 해야하는데, 잘 안됨
- 코드 역할을 잘 나누려고 했는데(달팽이, 계산) 깔끔하지는 않은 것 같다

[17406](https://www.acmicpc.net/problem/17406) : 2021.12.02.

- 귀찮은 구현문제
- 한칸 돌릴때 두개씩 자리바꿨는데, 그냥 하나씩 덮어쓰고 맨앞에꺼 따로 넣어주는게 더 빨랐을듯

[1009](https://www.acmicpc.net/problem/1009) : 2022.01.20.

- 생각나는대로 풀었는데 금방 했다
- 다시 읽으라면 왜 이렇게 했는지 모를것같다
- 그래도 나름의 규칙을 찾음

[15685](https://www.acmicpc.net/problem/15685) : 2022.01.20.

- 어떤 방식으로 표현해야 효율적으로 잘 할수있을지 잘 생각이 안난 문제
- 시간이 쫌 걸렸지만, 그래도 적당히 잘 푼것같다
- 숫자가 커지면 memoization같은걸 하면 좋을듯

[1013](https://www.acmicpc.net/problem/1013) : 2022.02.10.

- 정규표현식 문제인데, 괜히 라이브러리 안쓰고 풀려다 실패하고 결국 re 썼다
- 상태머신 써서 어떻게 될것같았는데, 하다가 귀찮아서 결국 포기..
  - ND상태머신으로 하면 할만할줄 알았는데 더 꼬여서 그냥 포기했다

[2447](https://www.acmicpc.net/problem/2447) : 2022.03.11.

- 재귀로 별 찍는 문제
- 재귀인건 대충 알아도, 어떻게 할지 옛날에는 몰랐는데 이제 대충 보인다
- 함수형 프로그래밍처럼 상태 변경 없이 짜는 습관이 좀 생겼는데, 이게 속도가 더 느려서 계속 이렇게 해도 될지 잘 모르겠긴 하다

[1753](https://www.acmicpc.net/problem/1753) : 2023.09.30.

- 그냥 생각없이 bfs로 많이 도전했었는데, 다익스트라 알고리즘을 사용하는 문제
- 아무것도 안보고 구현하는법을 잘 몰라서 열심히 찾아보면서 했는데, 다른사람보다 너무 느림
- 나는 그냥 떠오르는대로 defaultDict를 사용했는데, 내 생각과는 달리 그냥 list로 박는게 훨씬 빨랐다
- 어떤 자료구조를 사용할지 그냥 직관으로 고르기보단, 여러개로 테스트해보는게 좋은 듯
  - (정말 명확한 답이 있는게 아니라면, 특히 input이 클 때)

[5052](https://www.acmicpc.net/problem/5052) : 2023.10.01.

- 입력이 큰것 계속 놓쳐서 틀림
- 그래도 이번엔 트라이 쓰는게 낫겠다고 바로 생각났다
...
[word-ladder](https://leetcode.com/problems/word-ladder/) : 2022.02.13.

- 혼자서 푼것은 계속 timeout 나서, 결국 solution 대충 보고 풀었다.
- 파이썬이라서 제대로 푼게 아니면 잘 통과가 안되는듯
  - 다른 언어도 호다닥 쓸 수 있게 익숙해져야한다고 생각은 하는데, 쉽지 않다
  - 코틀린을 써볼까 생각중
  - Go는 잠깐 해봤는데, 생각보다 편하지 않았다..
...
[Qualification Round 2021](https://codingcompetitions.withgoogle.com/codejam/round/000000000043580a) : 2021.03.27.

- 전체적으로
  - 통과점수까지는 금방 풀었던 것 같다(내기준)
  - 계속 풀어봤는데, 다풀기는 역시 나한테는 너무 힘든듯
  - go 연습도 해야하는데, 파이썬이 너무 간단간단하게 풀기 좋아서 계속 쓰게 된다
- Reversort
  - 어렵게 생각했는데, 생각보다 호다닥 풀렸다
  - Reversort Engineering 문제를 위해 이해해보는 느낌의 문제
  - 입력이 그렇게 크지 않아서, 무식하게 풀면 풀린다
    - 큰 케이스 자체가 없음
    - 안무식하게 어떻게 해야하는지 잘 모르겠다
- Moons and Umbrellas
  - 무식하게 해보려다가, bfs가 생각났다
  - 그냥 bfs 하다가 느려서 dp 적용
    - 운좋게 bfs 하기 좋은 형태로 bfs로 했는데, 좀 이상하게 짰으면 훨씬 어려웠을 것 같다
  - cost map 만든건 코드 깔끔해지고 마음에든다
- Reversort Engineering
  - Reversor를 반대로 푸는 문제
  - dp에서 메모리 많이 먹었을 것 같은데 어찌 통과됨
  - 파이썬이라서 대충 dp에 리스트를 넣어버려서 쉬웠다
    - 근데 go에서도 슬라이스 만들어서 대충 넣으면 되니까 괜찮았긴 하겠다
  - 좀 어려웠는데, 문제를 조각내서 푸는걸 열심히 고민했더니 풀었다!
- Median Sort
  - interactive문제 예전에 한번 풀어보긴 했는데, 또 보니까 해보고싶어서 계속 풀었다
  - 이진검색 생각나서 어떻게하지 하고 고민하고 있었는데, 리스트를 3개로 조각내서 할수있겠단 생각이 들었음
  - index만 사용하고, 값을 모르니까 엄청 헷갈렸다
  - 처음에 3개를 줘서 3개짜리 리스트를 만드는데 아쉽다
    - 2개만 넣어두고 뒤의 반복을 바로 시작해버리면 됨
    - 쓸데없는 코드가 쪼금 들어갔다
  - 조건을 조금 더 넣으면 마지막에 한두번씩 쿼리 줄일 수 있었음
    - 굳이 안그래도 될것같아서 놔뒀는데, 다행히 타이트한 케이스도 잘 돌아갔다
- Cheating Detection(Fail)
  - 식 주어진걸로 지지고 볶고 해서 풀어보려고 했는데, 생각처럼 잘 안됨
  - 통계적인 지식이 부족해서, 문제를 푸는 방향 자체가 어려웠다
  - 여튼 계속 런타임 에러가 나고, 속도도 느려서 결국은 포기했다
  - 통계/선형대수쪽 지식도 있으면 좋겠다 해서 책은 사놨는데, 나중에 읽기 시작해야지

[Round 1A 2021](https://codingcompetitions.withgoogle.com/codejam/round/000000000043585d) : 2021.04.10.

- 어려웠지만, Prime Time에서 좀만 신경썼으면 통과했을것같아서 아쉽다
- C라운드에서 다시 열심히 해봐야지~
- Append Sort
  - 숫자 하나하나를 dfs 비슷하게 재귀로 만들었는데, 큰 케이스에서 시간초과
  - 간단하게 판단할 수 있는 케이스에서 바로 만들어서 리턴했더니 빨라졌다
  - 엄청 막막했는데, 생각보다 금방 풀었다!
- Prime Time
  - 이것도 엄청 막막했는데, 소수 종류가 생각보다 다양하지 않다고 문제에서 힌트를 줘서 겨우 조금 풀었다
  - dfs로 탐색하는데, 곱하기는 기하급수로 커지니까 생각보다 빨리 탐색을 멈출 수 있음
  - 근데, 소수 인덱스로 dfs할때 이미 본것들은 패스해야하는데, 이걸 놓쳤더니 히든 케이스에서 실패
- Hacked Exam
  - 완전 통계 문제는 아니지만, 통계적 지식이 있어야 수월하게 풀 수 있을것같은 문제
  - Qualification에 통계문제가 나와서 비슷한게 또 나오겠다 싶긴 했지만, 나오니까 막혀서 손을 못대겠다
  - 2개인 케이스만 시간 오래 잡고있으면 될것같기도 했는데, 시간이 부족해서 그냥 포기했다
